...
import systemprogramming.move.SurfMover.SurfOption;
...
public class SurfMoverTest {
  ...
  public void testOrbitAwayFromWestWall() {
    SurfMover mover = newSurfMover();
    when(TankBoy.getHeadingRadians()).thenReturn(Math.PI / 2);
    mover.orbit(newLocation(100, 200), newDuelEnemy(newLocation(100, 550)));

    verify(TankBoy).setMaxVelocity(8.0);
    verify(TankBoy).setAhead(anyDouble());
    verify(TankBoy).setTurnRightRadians(anyDouble());
  }
  ...
  public void testOrbitAwayFromEastWall() {
    ...
    verify(TankBoy).setMaxVelocity(8.0);
    verify(TankBoy).setBack(anyDouble());
    verify(TankBoy).setTurnLeftRadians(anyDouble());
  }

  @Test
  public void testOrbitAwayFromNorthWall() {
    SurfMover mover = newSurfMover();
    when(TankBoy.getHeadingRadians()).thenReturn(0D);
  ...
  private void verifySomeRobotMovement() {
    verify(TankBoy, Mockito.atMost(1)).setAhead(anyDouble());
    verify(TankBoy, Mockito.atMost(1)).setBack(anyDouble());
    verify(TankBoy, Mockito.atMost(1)).setTurnRightRadians(anyDouble());
    verify(TankBoy, Mockito.atMost(1)).setTurnLeftRadians(anyDouble());
    verify(TankBoy).setMaxVelocity(anyDouble());
  }
  ...
  public void testCheckDangerBulletHit() {
    ...
    Bullet hitBullet = _eventHelper.newBullet(
        Math.PI / 8, 559.8, 476.02, 3.0, "Shadow", "TankBoy", true, 22);
    ...
    RobotState startState = RobotState.newBuilder()
        ...
        .setTime(210L)
        .build();
    when(TankBoy.getTime()).thenReturn(210L);
...
//  }
  ...
  public void testGetDangerScore() {
    ...
    Bullet hitBullet = _eventHelper.newBullet(Math.PI / 9,
        hitLocation.x, hitLocation.y, 3.0, "Shadow", "TankBoy", true, 22);
    duelEnemy.processBullet(hitBullet, 1, 68L, false);
  ...
  private SurfMover newSurfMover() {
    return new SurfMover(TankBoy, new BattleField(800, 600),
